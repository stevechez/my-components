import React from 'react'
import type { NextPage } from 'next'

const Team2: NextPage = () => {
  return (
    <>
      {/* <!-- Section 1 --> */}
      <section className="relative w-full bg-white">
        <div className="absolute inset-0 w-full h-full opacity-25 sm:opacity-50">
          <svg
            className="absolute top-0 right-0 w-auto h-full opacity-75"
            viewBox="0 0 150 350"
            xmlns="http://www.w3.org/2000/svg"
          >
            <defs>
              <path d="M0 0h50v50H0z"></path>
              <path d="M0 0h50v50H0z"></path>
              <path d="M0 0h150v150H0z"></path>
            </defs>
            <g stroke="none" stroke-width="1" fill="none" fill-rule="evenodd">
              <g>
                <g fill="#DEB9FF" fill-rule="nonzero">
                  <path d="M25 25c13.807 0 25-11.193 25-25H0c0 13.807 11.193 25 25 25z"></path>
                </g>
                <g transform="translate(0 100)">
                  <mask fill="#fff"></mask>
                  <path
                    d="M25 0c13.807 0 25 11.193 25 25S38.807 50 25 50H0V0h25z"
                    fill="#F9C7FF"
                    mask="url(#mask-2)"
                  ></path>
                </g>
                <g transform="translate(100 100)">
                  <mask fill="#fff"></mask>
                  <path
                    d="M25 25c13.807 0 25-11.193 25-25H0c0 13.807 11.193 25 25 25z"
                    fill="#93FFFD"
                    fill-rule="nonzero"
                    mask="url(#mask-4)"
                  ></path>
                </g>
                <g transform="translate(0 200)">
                  <mask fill="#fff"></mask>
                  <path
                    d="M75 75c0 41.421 33.579 75 75 75V0c-41.421 0-75 33.579-75 75z"
                    fill="#93FFFD"
                    fill-rule="nonzero"
                    mask="url(#mask-6)"
                  ></path>
                </g>
              </g>
            </g>
          </svg>
          <svg
            className="absolute top-0 left-0 w-auto h-full opacity-30"
            viewBox="0 0 150 150"
            xmlns="http://www.w3.org/2000/svg"
          >
            <defs>
              <path d="M0 0h50v50H0z"></path>
              <path d="M0 0h50v50H0z"></path>
            </defs>
            <g stroke="none" stroke-width="1" fill="none" fill-rule="evenodd">
              <g transform="matrix(-1 0 0 1 150 0)">
                <g
                  transform="translate(0 25)"
                  fill="#93FFFD"
                  fill-rule="nonzero"
                >
                  <path d="M25 0C11.193 0 0 11.193 0 25h50C50 11.193 38.807 0 25 0z"></path>
                </g>
                <path
                  d="M25 50C11.193 50 0 38.807 0 25S11.193 0 25 0h25v50H25z"
                  transform="translate(100)"
                  fill="#DEB9FF"
                ></path>
                <g transform="translate(0 100)">
                  <mask fill="#fff"></mask>
                  <path
                    d="M25 50C11.193 50 0 38.807 0 25S11.193 0 25 0h25v50H25z"
                    fill="#B3EBFF"
                    mask="url(#mask-2)"
                  ></path>
                </g>
                <g transform="translate(100 200)">
                  <mask fill="#fff"></mask>
                  <path
                    d="M25 50C11.193 50 0 38.807 0 25S11.193 0 25 0h25v50H25z"
                    fill="#93FFFD"
                    mask="url(#mask-4)"
                  ></path>
                </g>
              </g>
            </g>
          </svg>
        </div>
        <div className="relative w-full px-8 pt-16 pb-16 ml-auto mr-auto bg-top bg-cover sm:max-w-xl md:max-w-full md:px-24 lg:px-8 lg:py-24 lg:pb-32">
          <div className="max-w-xl mb-10 ml-auto mr-auto bg-top bg-cover md:mx-auto sm:text-center lg:max-w-2xl md:mb-12">
            <p className="inline-block px-3 py-1 mb-4 text-xs font-semibold tracking-wider uppercase bg-purple-600 rounded-full text-purple-50">
              Our Team
            </p>
            <div className="max-w-2xl mb-6 ml-auto mr-auto font-sans tracking-tight text-gray-900 bg-top bg-cover sm:text-4xl md:mx-auto">
              <p className="inline max-w-lg font-sans text-3xl font-bold leading-none tracking-tight text-gray-900 sm:text-4xl md:mx-auto">
                Our Awesome Team of Unique Individuals
              </p>
            </div>
            <p className="text-base text-gray-700 md:text-lg">
              These are the amazing people that make us unique and stand-out
              amongst the competition. We are a team of focused and talented
              individuals.
            </p>
          </div>
          <div className="grid gap-10 row-gap-8 ml-auto mr-auto sm:row-gap-10 lg:max-w-screen-lg sm:grid-cols-2 lg:grid-cols-3">
            <div className="flex items-center">
              <img
                src="https://cdn.devdojo.com/images/july2021/03-Mike-Melcher.jpg"
                className="object-cover w-20 h-20 mr-4 rounded-full shadow"
              />
              <div className="relative">
                <p className="text-lg font-bold text-gray-800">Mike Melcher</p>
                <p className="text-sm text-gray-600">CEO &amp; Founder</p>
              </div>
            </div>
            <div className="flex items-center">
              <img
                src="https://cdn.devdojo.com/images/july2021/01-John-Cremson.jpg"
                className="object-cover w-20 h-20 mr-4 rounded-full shadow"
              />
              <div className="relative">
                <p className="text-lg font-bold text-gray-800">John Cremson</p>
                <p className="text-sm text-gray-600">Co-Founder</p>
              </div>
            </div>
            <div className="flex items-center">
              <img
                src="https://cdn.devdojo.com/images/july2021/02-Sally-Jones.jpg"
                className="object-cover w-20 h-20 mr-4 rounded-full shadow"
              />
              <div className="relative">
                <p className="text-lg font-bold text-gray-800">Sally Jones</p>
                <p className="text-sm text-gray-600">Marketing Manager</p>
              </div>
            </div>
            <div className="flex items-center">
              <img
                src="https://cdn.devdojo.com/images/july2021/04-Barbara-Sanchez.jpg"
                className="object-cover w-20 h-20 mr-4 rounded-full shadow"
              />
              <div className="relative">
                <p className="text-lg font-bold text-gray-800">
                  Barbara Sanchez
                </p>
                <p className="text-sm text-gray-600">SEO Specialist</p>
              </div>
            </div>
            <div className="flex items-center">
              <img
                src="https://cdn.devdojo.com/images/july2021/05-Sam-Lee.jpg"
                className="object-cover w-20 h-20 mr-4 rounded-full shadow"
              />
              <div className="relative">
                <p className="text-lg font-bold text-gray-800">Sam Lee</p>
                <p className="text-sm text-gray-600">Lead Designer</p>
              </div>
            </div>
            <div className="flex items-center">
              <img
                src="https://cdn.devdojo.com/images/july2021/06-Carly-Jenson.jpg"
                className="object-cover w-20 h-20 mr-4 rounded-full shadow"
              />
              <div className="relative">
                <p className="text-lg font-bold text-gray-800">Carly Jenson</p>
                <p className="text-sm text-gray-600">Designer</p>
              </div>
            </div>
            <div className="flex items-center">
              <img
                src="https://cdn.devdojo.com/images/july2021/07-Jeremy-Deen.png"
                className="object-cover w-20 h-20 mr-4 rounded-full shadow"
              />
              <div className="relative">
                <p className="text-lg font-bold text-gray-800">Jermy Deen</p>
                <p className="text-sm text-gray-600">Sales Manager</p>
              </div>
            </div>
            <div className="flex items-center">
              <img
                src="https://cdn.devdojo.com/images/july2021/08-Susan-Miller.jpg"
                className="object-cover w-20 h-20 mr-4 rounded-full shadow"
              />
              <div className="relative">
                <p className="text-lg font-bold text-gray-800">Susan Miller</p>
                <p className="text-sm text-gray-600">Developer</p>
              </div>
            </div>
            <div className="flex items-center">
              <img
                src="https://cdn.devdojo.com/images/july2021/paul-richards.jpg"
                className="object-cover w-20 h-20 mr-4 rounded-full shadow"
              />
              <div className="relative">
                <p className="text-lg font-bold text-gray-800">Paul Denson</p>
                <p className="text-sm text-gray-600">Developer</p>
              </div>
            </div>
          </div>
        </div>
      </section>
    </>
  )
}

export default Team2
